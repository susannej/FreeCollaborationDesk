...
if (isset($_POST['update'])){
 if ( $_POST['nacl'] == md5(AUTH_KEY.$db->get_var("select user_password from site_users where user_id = $user_id;")) ) {
	...
	if ( $user_id == $db->get_var("select note_post_user from site_notes where note_post_user = $user_id;") ) {
		$note_body = trim(addslashes($_POST['note_body']));
...
//	$note_title = addslashes($_POST['note_title']);
	$note_body = trim(addslashes($_POST['note_body']));
...
<!-- 	<tr><td valign="top">Title: </td><td><input type="text" name="note_title" size="43"></td></tr> -->
	<tr><td><textarea cols="65" rows="6" name="note_body"><?php echo $note_body;?></textarea></td></tr>
	<tr><td colspan='1'><input type='submit' name='update' value='update' class='btn btn-primary'></td></tr>
</form>
</table>
<h5><i class="icon-arrow-left"></i> <a href="fhd_call_edit.php?call_id=<?php echo $call_id;?>">back to ticket details</a></h5>
<?php } 
...
<!-- 	<tr><td valign="top">Title: </td><td><input type="text" name="note_title" size="43"></td></tr> -->
	<tr><td><textarea rows="4" name="note_body" class="input-xlarge"></textarea></td></tr>
	...
	<?php if(isset($_SESSION['admin'])){ ?>
		<h5><i class="icon-arrow-left"></i> <a href="fhd_call_edit.php?call_id=<?php echo $call_id;?>">back to ticket details</a></h5>
		<?php }else{ ?>
		<h5><i class="icon-arrow-left"></i> <a href="fhd_call_details.php?call_id=<?php echo $call_id;?>">back to ticket details</a></h5>
	<?php } ?>
